...
def plot_profile_data(df, gene_name, profile_type, output_dir):
    """
    Generates and saves a plot from the combined dataframe, comparing a specific profile type.

    Args:
        df (pd.DataFrame): The combined dataframe to plot.
        gene_name (str): The gene name for labeling.
        profile_type (str): The profile type to plot (e.g., 'Norm_profile').
        output_dir (str): The directory to save the plot in.
    """
    profile_cols = [col for col in df.columns if col.startswith(f'{profile_type}_')]

    if not profile_cols:
        logging.warning(f"No '{profile_type}' columns found for {gene_name} to plot.")
        return
    ...
    for col_name in profile_cols:
        ...
        treatment_name = col_name.replace(f'{profile_type}_', '')
        plt.plot(df.index, df[col_name], label=treatment_name)

    plot_title_type = profile_type.replace('_', ' ').title()
    plt.title(f'Combined {plot_title_type}s for {gene_name}')
    plt.xlabel('Nucleotide Position')
    plt.ylabel(f'Value ({profile_type})')
    plt.legend(title='Treatments')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.tight_layout()

    plot_filename = f"{gene_name}_{profile_type}_plot.png"
    plot_filepath = os.path.join(output_dir, plot_filename)

    try:
        plt.savefig(plot_filepath)
        logging.info(f"Successfully saved {profile_type} plot to {plot_filepath}")
...
def calculate_and_save_sliding_window(df, gene_name, profile_type, output_dir):
    """
    Calculates a sliding window normalization for a specific profile type and saves to a TSV.
    """
    profile_cols = [col for col in df.columns if col.startswith(f'{profile_type}_')]
    
    if not profile_cols:
        logging.info(f"No '{profile_type}' columns found for {gene_name} to calculate sliding window.")
        return None

    logging.info(f"Calculating sliding window for {profile_type} for {gene_name}...")
    ...
    for col_name in profile_cols:
        ...
        new_col_name = f"SlidingWindow_{col_name.replace(f'{profile_type}_', '')}"
        sliding_window_df[new_col_name] = series / rolling_median

    output_filename = f"{gene_name}_{profile_type}_sliding_window.tsv"
...
def plot_sliding_window_data(df, gene_name, profile_type, output_dir):
    """
    Generates and saves a plot from the sliding window dataframe.
    """
    if df is None or df.empty:
        logging.warning(f"Sliding window dataframe for {profile_type} is empty for {gene_name}. Skipping plot.")
        return
        
    sliding_window_cols = [col for col in df.columns if col.startswith('SlidingWindow_')]

    if not sliding_window_cols:
        logging.warning(f"No 'SlidingWindow' columns found for {profile_type} for {gene_name} to plot.")
        return
    ...
    plot_title_type = profile_type.replace('_', ' ').title()
    plt.title(f'Sliding Window ({plot_title_type}) for {gene_name}')
    plt.xlabel('Nucleotide Position')
    ...
    plot_filename = f"{gene_name}_{profile_type}_sliding_window_plot.png"
    plot_filepath = os.path.join(output_dir, plot_filename)
...
def calculate_and_plot_correlation(df, gene_name, profile_type, output_dir):
    """
    Calculates, saves, and plots the correlation matrix for a specific profile type.
    """
    profile_cols = [col for col in df.columns if col.startswith(f'{profile_type}_')]

    if len(profile_cols) < 2:
        logging.info(f"Skipping correlation for {profile_type} for {gene_name}: less than two columns.")
        return

    logging.info(f"Calculating correlation matrix for {profile_type} for {gene_name}...")
    
    correlation_df = df[profile_cols].corr(method='pearson')

    clean_names = [col.replace(f'{profile_type}_', '') for col in correlation_df.columns]
    correlation_df.columns = clean_names
    correlation_df.index = clean_names

    output_table_name = f"{gene_name}_{profile_type}_correlation.tsv"
    ...
    plot_title_type = profile_type.replace('_', ' ').title()
    plt.title(f'Correlation of {plot_title_type}s for {gene_name}')
    plt.xticks(rotation=45, ha="right")
    plt.yticks(rotation=0)
    plt.tight_layout()

    plot_filename = f"{gene_name}_{profile_type}_correlation_heatmap.png"
    plot_filepath = os.path.join(output_dir, plot_filename)
...
def run_profile_specific_analysis(df, gene_name, profile_type, output_dir):
    """
    A wrapper function to run all analyses specific to a profile type.

    Args:
        df (pd.DataFrame): The combined dataframe containing all data.
        gene_name (str): The gene name for labeling.
        profile_type (str): The profile type to analyze (e.g., 'Norm_profile').
        output_dir (str): The specific output directory for this analysis.
    """
    logging.info(f"--- Running Analysis for {profile_type} ---")
    
    # Calculate and save sliding window normalization
    sliding_window_df = calculate_and_save_sliding_window(df, gene_name, profile_type, output_dir)

    # Plotting and Analysis Steps
    calculate_and_plot_correlation(df, gene_name, profile_type, output_dir)
    plot_profile_data(df, gene_name, profile_type, output_dir)
    plot_sliding_window_data(sliding_window_df, gene_name, profile_type, output_dir)
...
def process_gene_group(gene_name, dir_info_tuples, base_output_dir):
    ...
    # Create the three specific subdirectories for the analysis outputs
    norm_profile_dir = os.path.join(output_gene_dir, "Norm_profile")
    reactivity_profile_dir = os.path.join(output_gene_dir, "Reactivity_profile")
    probing_efficiency_dir = os.path.join(output_gene_dir, "Probing_efficiency")
    os.makedirs(norm_profile_dir, exist_ok=True)
    os.makedirs(reactivity_profile_dir, exist_ok=True)
    os.makedirs(probing_efficiency_dir, exist_ok=True)

    combined_df = None
    base_cols = ["Nucleotide", "Sequence"]
    # Add 'Reactivity_profile' to the columns to extract
    data_cols_to_extract = ["Modified_rate", "Untreated_rate", "Norm_profile", "Reactivity_profile"]
    ...
    for dir_path, info_dir_name in sorted(dir_info_tuples):
        ...
        try:
            ...
            if not all(col in temp_df.columns for col in required_cols):
                logging.warning(f"Skipping {profile_file}: missing one or more required columns ({required_cols}).")
                continue
    ...
    if combined_df is not None and not combined_df.empty:
        ...
        reactivity_profile_cols = sorted([col for col in combined_df.columns if col.startswith('Reactivity_profile_')])
        
        # Add Reactivity profiles to the final combined table
        new_column_order = (base_cols + modified_rate_cols + untreated_rate_cols + 
                            norm_profile_cols + reactivity_profile_cols)
        combined_df = combined_df[new_column_order]

        # Save the main combined data table in the parent gene directory
        ...
        # --- Analyses that are NOT profile-specific ---
        # These will be saved in the Probing_efficiency directory.
        logging.info("--- Running Rate-Specific Analyses (saving to Probing_efficiency folder) ---")
        summary_data = []
        for nucleotide in ['A', 'U', 'C', 'G']:
            ...
            for col in modified_rate_cols + untreated_rate_cols:
                ...
                if '50%' in stats_series: stats_series = stats_series.rename({'50%': 'median'})
                stats = stats_series.to_dict()
                parts = col.split('_', 1)
                rate_type, treatment = parts[0], parts[1].replace('rate_', '')
                row = {'Nucleotide': nucleotide, 'Rate_Type': rate_type, 'Treatment': treatment, **stats}
                summary_data.append(row)
        
        summary_df = pd.DataFrame(summary_data)
        if not summary_df.empty:
            summary_table_path = os.path.join(probing_efficiency_dir, f"{gene_name}_nucleotide_summary.tsv")
        ...
        plot_box_plots(combined_df, gene_name, probing_efficiency_dir)
        plot_nucleotide_boxplots(combined_df, gene_name, probing_efficiency_dir)

        # --- Run Profile-Specific Analyses ---
        run_profile_specific_analysis(combined_df, gene_name, 'Norm_profile', norm_profile_dir)
        run_profile_specific_analysis(combined_df, gene_name, 'Reactivity_profile', reactivity_profile_dir)
